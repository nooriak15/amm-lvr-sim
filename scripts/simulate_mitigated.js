...
// Flag trades where AMM is stale relative to external price
function isToxic(ammPrice, externalPrice, threshold = 0.01) {
    const gap = Math.abs(ammPrice - externalPrice) / externalPrice;
...
if (isBuy) {
    ...
    // Reverse swap (simulate manually)
    ...
    reserve0 -= dx;
        ...
        // Check toxicity before arbitrage
        const ammPriceBeforeArb = getAMMPrice(reserve0, reserve1);
        let toxic = false;

        if (isToxic(ammPriceBeforeArb, externalPrice)) {
            toxic = true;
            console.log(`⚠️ Toxic trade at trade ${i}. Soft mitigation triggered.`);
            // Gently nudge external price toward AMM price (5%)
            const blend = 0.05;
            externalPrice = externalPrice * (1 - blend) + ammPriceBeforeArb * blend;
        }
        ...
        const ammPriceAfterArb = getAMMPrice(reserve0, reserve1);
        const block = await ethers.provider.getBlock("latest");

        data.push({
            tradeNumber: i,
            timestamp: block.timestamp,
            amm_price: ammPriceAfterArb,
            external_price: externalPrice,
            trade_size: noiseTradeAmount,
            toxic_trade: toxic
        });

        console.log(`Trade ${i}: noise=${noiseTradeAmount}, amm=${ammPriceAfterArb.toFixed(6)}, external=${externalPrice.toFixed(6)}`);
    }

    fs.writeFileSync("trade_data_mitigated.json", JSON.stringify(data, null, 2));
    console.log("✅ trade_data_mitigated.json saved");
    ...
    plot([traceAMM, traceExternal], {
        title: 'AMM vs External Price with Mitigation',
